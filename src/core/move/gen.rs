...
pub fn least_set_one_bit(board: Bits) -> Bits {
  ...
  (board as i64 & -dbg!(board as i64)) as u64
}
...
fn generate_white_legal_target_bitboards(pos: &Position) -> TargetBoards {
  ...
  let _null_if_check: Bits = (((attacks & pos.white.king) as i64 - 1) as i64 >> 63) as u64;
  let _null_if_double_check: Bits =
    (((_check_from & ((_check_from as i64) - 1) as u64) as i64 - 1) as i64 >> 63) as u64;
...
pub fn generate_legal_moves(_pos: &Position) -> MoveList {
  ...
  println!(
    "generate_legal_moves: generating legal target BBs for position:\n{}",
    pos.to_string()
  );
  let move_target_bitboards = generate_white_legal_target_bitboards(&pos);
  println!(
    "generate_legal_moves: move target bitboards\n{}",
    Board::bits_to_string(move_target_bitboards.all()),
  );
  ...
  // north north west
  println!("north north west");
  let mut nnw_move_targets = move_target_bitboards.nnw;
  let mut iter = 0;
  while nnw_move_targets != 0 {
    println!(
      "generate_legal_moves: nnw_move_targets[{}]\n{}",
      iter,
      Board::bits_to_string(nnw_move_targets)
    );
    iter += 1;
