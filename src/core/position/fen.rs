...
impl Position {
        ...
        match stage {
          0 => {
            ...
            match c.to_digit(10) {
              ...
              None => {
                let index: u8 =
                  (2 * ((i as i32 + skip_offset) % 8) + 56 - i as i32 - skip_offset) as u8;
                match c {
          ...
          3 => {
            ...
            if c.is_alphabetic() && ep_file.is_none() {
              ep_file = Some(c.to_ascii_lowercase() as u8 - 'a' as u8);
            } else if ep_file.is_some() {
              match c.to_digit(10) {
                Some(x) => {
                  pos.en_passant = (x as BoardIndex - 1) * 8 + ep_file.unwrap() as BoardIndex;
                }
                None => {
                  return Err(format!(
  ...
  pub fn to_fen(&self) -> String {
    ...
    for i in 0..64 {
      ...
      if blanks != 0 {
        if get_bit(self.get_all_pieces(), index) != 0 || i % 8 == 0 {
